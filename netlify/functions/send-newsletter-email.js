...
exports.handler = async function(event, context) {
  ...
  const ADMIN_EMAIL_ADDRESS = 'omotoyinbobade15@gmail.com'; // Your admin email
  ...
  try {
    ...
    const welcomeEmailPayload = {
      from: 'Acme <onboarding@resend.dev>', // Using Resend's default from for now
      to: [userEmail],
    ...
    if (!welcomeResponse.ok) {
      ...
      return {
        statusCode: welcomeResponse.status, // Forward Resend's status code
        body: JSON.stringify({ message: 'Failed to send welcome email.', details: errorData }),
        headers: { 'Content-Type': 'application/json' },
      };
    }
    console.log(`Welcome email sent to ${userEmail} successfully.`);
    ...
    if (!notificationResponse.ok) {
      ...
      console.error(`Failed to send admin notification email for ${userEmail}:`, errorData);
      ...
      console.log(`Admin notification for ${userEmail} sent successfully.`);
    }
